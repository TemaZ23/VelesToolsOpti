...
import type { BotDepositConfigDto, BotProfitConfigDto, BotStopLossConfigDto, StrategyConditionDto } from './bots.dtos';
...
export interface BotStrategy {
  ...
  // Дополнительные поля, возвращаемые API для полных данных бота
...
export const postBacktest = async (payload: BotStrategy): Promise<BacktestCreateResponse> => {
  ...
  if (!response.ok) {
    ...
    console.error('[postBacktest] Error response:', {
